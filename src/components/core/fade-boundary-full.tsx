...
export type FadeBoundaryFullProps = {
  side?: "top" | "bottom";
  startAt?: number;
  endAt?: number;
  className?: string;
 ...
 */
export function FadeBoundaryFull({ side = "bottom", startAt, endAt, className }: FadeBoundaryFullProps) {
  ...
  const gradientClass = side === "bottom"
    ...
    : "bg-gradient-to-t from-transparent via-white/100 to-white";
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={[
        "pointer-events-none absolute h-[80%] z-90",
        positionClass,
        className ?? "",
      ].join(" ")}
      style={{ opacity: "var(--fade)" }}
    >
      <div className={["absolute inset-1", gradientClass].join(" ")} />
      <div className="absolute inset-0 bg-white/90 blur-3xl" />
    </div>
  );
}
