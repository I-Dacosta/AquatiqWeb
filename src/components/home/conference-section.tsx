...
export function ConferenceSection() {
  ...
  const [topRightPrimary, topRightSecondary, ...remainingEvents] =
    nonFeaturedEvents;
  ...
  return (
    <section
      id="conference"
      ref={containerRef}
      className={`w-full py-12 md:py-16 lg:py-20 border-t transition-colors duration-700 ${
        isDark
    ...
    >
      ...
      <div className="px-[6vw] mb-8 md:mb-10">
        <div className="news-header flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div>
            <span
              className={`news-header-element block mb-5 text-[10px] font-bold tracking-[0.25em] uppercase transition-colors duration-700 ${isDark ? "text-white/50" : "text-[#1a1d1d]/50"}`}
            >
              05 / Aktuelt
            </span>
          ...
          <div className="max-w-xs flex flex-col gap-5">
            <p
              className={`news-header-element text-sm leading-relaxed transition-colors duration-700 ${isDark ? "text-white/55" : "text-[#1a1d1d]/55"}`}
            >
              Nyeste Eventer
            </p>
      ...
      <div className="px-[6vw]">
        <div className={`grid grid-cols-1 md:grid-cols-3 border-t border-l transition-colors duration-700 ${isDark ? 'border-white/15' : 'border-[#1a1d1d]/15'}`}>
          ...
          <Link
            ...
            className={`conf-card group relative md:col-span-2 border-r border-b overflow-hidden flex flex-col justify-between min-h-70 md:min-h-95 transition-colors duration-700 ${isDark ? 'border-white/15 bg-[#1a1d1d] hover:bg-[#222525]' : 'border-[#1a1d1d]/15 bg-[#f6f6f6] hover:bg-[#e2e2e2]'}`}
          >
            {featured.image && (
          ...
          <div className={`flex flex-col border-b transition-colors duration-700 ${isDark ? 'border-white/15' : 'border-[#1a1d1d]/15'}`}>
            {topRightPrimary && (
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`conf-card group relative flex flex-col justify-between p-6 md:p-8 min-h-35 md:min-h-47.5 border-b border-r transition-colors duration-700 overflow-hidden ${isDark ? 'hover:bg-[#222525] border-white/15' : 'hover:bg-neutral-50 border-[#1a1d1d]/15'}`}
              >
            ...
            {topRightSecondary && (
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`conf-card group relative flex flex-col justify-between p-6 md:p-8 min-h-35 md:min-h-47.5 border-r transition-colors duration-700 overflow-hidden ${isDark ? 'hover:bg-[#222525] border-white/15' : 'hover:bg-neutral-50 border-[#1a1d1d]/15'}`}
              >
          ...
          {remainingEvents.map((ev) => (
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`conf-card group relative flex flex-col justify-between border-r border-b p-6 md:p-8 min-h-45 md:min-h-55 transition-colors duration-700 overflow-hidden ${isDark ? 'border-white/15 hover:bg-[#222525]' : 'border-[#1a1d1d]/15 hover:bg-neutral-50'}`}
            >
              {ev.hoverImage && (
                <Image
                  src={ev.hoverImage}
          ...
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="conf-card group flex flex-col justify-between border-r border-b p-6 md:p-8 min-h-45 md:min-h-55 md:col-span-2 transition-colors duration-700 border-white/15 bg-[#F6AF6E] text-black hover:bg-neutral-200"
          >
